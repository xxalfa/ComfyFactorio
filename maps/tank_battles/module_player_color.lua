    ...
    local function on_player_changed_position( event )
        ...
        for index, item in pairs( table_of_colored_tiles ) do
            ...
            if math.floor( player.position.x ) == math.floor( p.x ) and math.floor( player.position.y ) == math.floor( p.y )

            or math.ceil( player.position.x ) == math.ceil( p.x ) and math.ceil( player.position.y ) == math.ceil( p.y ) then
